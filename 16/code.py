...
My First Crime Scene Analysis Machine!  Just what you wanted.
Or needed, as the case may be.

The My First Crime Scene Analysis Machine (MFCSAM for short) can detect
a few specific compounds in a given sample,
...
class Sue(NamedTuple):
	...
	def from_string(cls, string: str):
		number, things = string[4:].split(": ", 1)
		...
		return cls(number=number, **kwargs)
	...
	def __eq__(self, other):
		...
		for prop, value in self._asdict().items():
			if prop == "number":
				continue
			if value == -1:
				continue

			if other_dict[prop] != value:
				return False
...
for line in [x for x in PathPlus("input.txt").read_lines() if x]:
	...
	if sue == gift_giver:
		print(f"The gift was given by Sue {sue.number}.")  # 373
		break
...
are greater than that many (due to the unpredictable nuclear decay of cat dander and tree pollen),
...
for line in [x for x in PathPlus("input.txt").read_lines() if x]:
	sue = SueV2.from_string(line)

	if sue == gift_giver:
		print(f"The gift was given by Sue {sue.number}.")  # 260
		break
