...
presents = [x for x in PathPlus("input.txt").read_lines() if x]

total = 0

for present in presents:
	l, w, h = sorted(map(int, re.match(r"(\d+)x(\d+)x(\d+)", present).groups()))
	sides = (l * w, w * h, h * l)

	surface_area = int(sum(sides) * 2)
	slack = min(sides)

	required = surface_area + slack
	total += required
...
print(f"The elves need {total} feet of ribbon.")
