...
def play_game(us, boss):

	while True:
		boss["hit"] -= max((us["damage"] - boss["armour"]), 1)
		if boss["hit"] <= 0:
			# print("You won")
			return True
		...
		us["hit"] -= max((boss["damage"] - us["armour"]), 1)
		if us["hit"] <= 0:
			# print("You lost")
			return False
...
us = {"hit": 100, "damage": 0, "armour": 0}
boss = {"hit": 100, "damage": 8, "armour": 2}
...
armour_sets = [
		Item("Leather", 13, armour=1),
		Item("Chainmail", 31, armour=2),
		Item("Splintmail", 53, armour=3),
		Item("Bandedmail", 75, armour=4),
		Item("Platemail", 102, armour=5),
		Item("No Armour", 0),
		]
...
ring_perms = list(
		itertools.combinations([
				...
				Item("Defence +1", 20, damage=1),
				Item("Defence +2", 40, damage=2),
				Item("Defence +3", 80, damage=3),
				Item("No Ring", 0),
				Item("No Ring", 0),
				],
								2)
...
for perm in ring_perms:
	...
	for weapon in weapons:
		# print(weapon)
		for armour in armour_sets:
			# print(armour)
			us = {"hit": 100, "damage": weapon.damage, "armour": armour.armour}
			boss = {"hit": 100, "damage": 8, "armour": 2}
			for ring in perm:
				us["armour"] += ring.armour
				us["damage"] += ring.damage

			if play_game(us, boss):
				combinations.append((weapon, armour, *perm))

winning_combination = min(combinations, key=lambda x: sum(map(attrgetter("cost"), x)))
min_gold_spent = sum(map(attrgetter("cost"), winning_combination))
print("The minimum amount of gold that can be spent to win is:", min_gold_spent)
...
for perm in ring_perms:
	...
	for weapon in weapons:
		...
		for armour in armour_sets:
			...
			if not play_game(us, boss):
				combinations.append((weapon, armour, *perm))

winning_combination = max(combinations, key=lambda x: sum(map(attrgetter("cost"), x)))
min_gold_spent = sum(map(attrgetter("cost"), winning_combination))
print("The minimum amount of gold that can be spent to win is:", min_gold_spent)
