...
On the console of the machine is a copy protection message asking you
to enter a code from the instruction manual.
...
"that's not even all of the first few numbers; for example,
you're missing the one at ``7,1`` that would come before ``6,2``.
...
row_idx, col_idx = 0, 0
last_value = codes[row_idx][col_idx]
...
while True:
	...
	while True:
		...
		if col_idx == 3083 - 1 and row_idx == 2978 - 1:
			break
...
# ==========================
"""
The machine springs to life, then falls silent again.
It beeps. "Insufficient fuel", the console reads.
